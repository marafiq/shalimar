...
public class ShalimarGenerator : IIncrementalGenerator
{
        ...
        ((((((Compilation Compilation, ImmutableArray<ComponentInfo?> Components) Left, ImmutableArray<DeferredInfo?> Deferred) Mid, ImmutableArray<LazyInfo?> Lazy) Left2, ImmutableArray<StreamInfo?> Stream) Left3, ImmutableArray<SseInfo?> Sse) Left4, ImmutableArray<MutationInfo?> Mutations) source)
    {
        ...
        GenerateTypeScriptFormsEmbedded(context, compilation, mutations);
    ...
    private static void GenerateTypeScriptFacadesEmbedded(SourceProductionContext context)
    {
        ...
        EmitFacade(context, "ShalimarFacade.Forms.g.cs", "forms.ts", "export * from './shalimar-forms.g'");
        EmitFacade(
            context,
    ...
    private static void GenerateTypeScriptFormsEmbedded(
        SourceProductionContext context,
        Compilation compilation,
        List<MutationInfo> mutations)
    {
        ...
        tsSb.AppendLine("import {");
        foreach (var r in requests.OrderBy(t => t.Name, StringComparer.Ordinal))
        ...
        tsSb.AppendLine("import type {");
        foreach (var t in requests.Concat(responses).OrderBy(t => t.Name, StringComparer.Ordinal))
            tsSb.AppendLine($"    {t.Name},");
        tsSb.AppendLine("} from './shalimar-types.g'");
        tsSb.AppendLine();

        tsSb.AppendLine("export type FormSpec<TReq, TRes> = {");
        ...
        tsSb.AppendLine("export const forms = {");
        foreach (var m in mutations.OrderBy(m => m.MethodName, StringComparer.Ordinal))
        {
            ...
            tsSb.AppendLine($"    {key}: {{ defaults: {req}Defaults, schema: {req}Schema, mutate: {m.MethodName}, serverKey: defaultServerKey, mapClientIssues: mapIssues }} satisfies FormSpec<{req}, {res}>,");
        }
        tsSb.AppendLine("} as const");
        tsSb.AppendLine();
        ...
        csSb.AppendLine("// <auto-generated/>");
        csSb.AppendLine("/*");
        csSb.AppendLine("SHALIMAR_TS: shalimar-forms.g.ts");
        csSb.Append(tsSb);
        csSb.AppendLine("END_SHALIMAR_TS");
        csSb.AppendLine("*/");

        context.AddSource("ShalimarForms.g.cs", SourceText.From(csSb.ToString(), Encoding.UTF8));
    }
