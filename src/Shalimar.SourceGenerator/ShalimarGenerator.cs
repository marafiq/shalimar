...
public class ShalimarGenerator : IIncrementalGenerator
{
        ...
        List<SseInfo> sse)
    {
        ...
        tsSb.AppendLine("import { invalidateDeferred, invalidateLazy, invalidateStreamed, invalidateSse } from '@shalimar/runtime'");
        tsSb.AppendLine();
        ...
        tsSb.AppendLine("    for (const href of inv.deferredHrefs) invalidateDeferred(href)");
        tsSb.AppendLine("    for (const href of inv.lazyHrefs) invalidateLazy(href)");
        tsSb.AppendLine("    for (const href of inv.streamedHrefs) invalidateStreamed(href)");
        tsSb.AppendLine("    for (const href of inv.sseHrefs) invalidateSse(href)");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        ...
        List<MutationInfo> mutations)
    {
        ...
        tsSb.AppendLine("async function jsonMutation<TReq, TRes>(url: string, method: string, body: TReq): Promise<TRes> {");
        ...
        tsSb.AppendLine("        throw new Error(`Mutation failed (${res.status}) ${method} ${url}\\n${text}`)");
        tsSb.AppendLine("    }");
        tsSb.AppendLine("    return (await res.json()) as TRes");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        ...
        foreach (var m in mutations)
        {
            ...
            tsSb.AppendLine($"export async function {fn}({paramSig}req: {m.RequestType.Name}): Promise<{m.ResponseType.Name}> {{");
            ...
            tsSb.AppendLine($"    const value = await jsonMutation<{m.RequestType.Name}, {m.ResponseType.Name}>(url, '{m.HttpMethod}', req)");
            foreach (var inv in m.InvalidatesComponentTypeNames)
            {
                tsSb.AppendLine($"    invalidateComponent('{inv}' as ComponentKey)");
            }
            tsSb.AppendLine("    return value");
            tsSb.AppendLine("}");
            tsSb.AppendLine();
        }
