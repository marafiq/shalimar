...
public class ShalimarGenerator : IIncrementalGenerator
{
        ...
        List<ComponentInfo> components)
    {
        ...
        tsSb.AppendLine();
        tsSb.AppendLine("export const v2Stores: { [K in V2ComponentKey]: Store<V2PropsByComponent[K] | null> } = {");
        foreach (var c in v2)
            tsSb.AppendLine($"    {c.TypeName}: new Store<V2PropsByComponent['{c.TypeName}'] | null>(null),");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function setV2Props<K extends V2ComponentKey>(key: K, props: V2PropsByComponent[K]) {");
        tsSb.AppendLine("    v2Stores[key].setState(props)");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function useV2Props<K extends V2ComponentKey>(key: K): V2PropsByComponent[K] {");
        tsSb.AppendLine("    const value = useStore(v2Stores[key])");
        tsSb.AppendLine("    if (!value) throw new Error(`Missing v2 props for ${String(key)}. Did the route loader run?`)");
        tsSb.AppendLine("    return value as V2PropsByComponent[K]");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function getV2Props<K extends V2ComponentKey>(key: K): V2PropsByComponent[K] | null {");
        tsSb.AppendLine("    return v2Stores[key].state as V2PropsByComponent[K] | null");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        ...
        List<ComponentInfo> components)
    {
        ...
        var tsSb = new StringBuilder();
        tsSb.AppendLine("// Generated by Shalimar - DO NOT EDIT");
        tsSb.AppendLine("import { useDeferred } from '@shalimar/runtime'");
