...
public class ShalimarGenerator : IIncrementalGenerator
{
        ...
        ((((((Compilation Compilation, ImmutableArray<ComponentInfo?> Components) Left, ImmutableArray<DeferredInfo?> Deferred) Mid, ImmutableArray<LazyInfo?> Lazy) Left2, ImmutableArray<StreamInfo?> Stream) Left3, ImmutableArray<SseInfo?> Sse) Left4, ImmutableArray<MutationInfo?> Mutations) source)
    {
        ...
        GenerateTypeScriptV2RouteModulesEmbedded(context, compilation, components);
        ...
        List<ComponentInfo> components)
    {
        ...
        var mappings = components
            .Select(c =>
            {
                ...
                var file = isV2
                    ? ToGeneratedPath(MapV2RouteModuleFile(normalized))
        ...
        // TanStack requires route modules for every segment that has children. If v2 contributes routes,
        // we generate layout modules under Generated/V2Routes/** for ancestor segments too (e.g. "/v2").
        var seen = new HashSet<string>(mappings.Select(m => m.Path), StringComparer.Ordinal);
        foreach (var v2Path in components
                     .Where(c => !string.IsNullOrWhiteSpace(c.TsxFile) &&
                                 c.TsxFile!.Replace('\\', '/').StartsWith("Features/V2/", StringComparison.Ordinal))
                     .Select(c => c.Path == "/" ? "/" : "/" + c.Path.Trim('/'))
                     .Where(p => p != "/")
                     .Distinct(StringComparer.Ordinal)
                     .OrderBy(p => p, StringComparer.Ordinal))
        {
            ...
            while (true)
            {
                var idx = parent.LastIndexOf('/', parent.Length - 1);
                if (idx <= 0) break;
                parent = parent.Substring(0, idx);
                if (parent == "/") break;

                if (seen.Add(parent))
                    mappings.Add((Path: parent, FilePath: ToGeneratedPath(MapV2RouteModuleFile(parent))));
            }
        }
        ...
        List<ComponentInfo> components)
    {
        ...
        // Also generate layout modules for ancestor segments under /v2 so TanStack doesn't require handwritten files.
        var v2LayoutPaths = new SortedSet<string>(StringComparer.Ordinal);
        foreach (var c in v2)
        {
            var normalized = c.Path == "/" ? "/" : "/" + c.Path.Trim('/');
            ...
            while (true)
            {
                ...
                parent = parent.Substring(0, idx);
                if (parent == "/") break;
                v2LayoutPaths.Add(parent);
            }
        }

        foreach (var layoutPath in v2LayoutPaths)
        {
            var embedded = MapV2RouteModuleFile(layoutPath);  // V2Routes/...
            var projectFile = ToGeneratedPath(embedded);      // Generated/V2Routes/...

            var tsSb = new StringBuilder();
            tsSb.AppendLine("// Generated by Shalimar - DO NOT EDIT");
            tsSb.AppendLine("import { createFileRoute, Outlet } from '@tanstack/react-router'");
            tsSb.AppendLine();
            tsSb.AppendLine($"export const Route = createFileRoute('{layoutPath}')({{");
            tsSb.AppendLine("    component: () => <Outlet />,");
            tsSb.AppendLine("})");
            tsSb.AppendLine();
            ...
            csSb.AppendLine("// <auto-generated/>");
            csSb.AppendLine("/*");
            csSb.AppendLine($"SHALIMAR_TS: {embedded}");
            csSb.Append(tsSb);
            csSb.AppendLine("END_SHALIMAR_TS");
            csSb.AppendLine("*/");

            context.AddSource($"V2Route_{SanitizeHintName(projectFile)}.g.cs", SourceText.From(csSb.ToString(), Encoding.UTF8));
        }

        foreach (var c in v2)
        {
            var normalized = c.Path == "/" ? "/" : "/" + c.Path.Trim('/');
            var embedded = MapV2RouteModuleFile(normalized);   // e.g. V2Routes/V2/Workbench/route.tsx
            var modulePath = ToGeneratedPath(embedded);        // e.g. Generated/V2Routes/V2/Workbench/route.tsx
            ...
            tsSb.AppendLine("import { prefetchBehaviors } from '@shalimar/runtime'");
            tsSb.AppendLine("import { setV2Props, useV2Props, v2Keys } from '@generated/store'");
            ...
            tsSb.AppendLine("    loader: () => {");
            tsSb.AppendLine($"        const props = (window as any).__SHALIMAR_PROPS__ as {typeName}");
            tsSb.AppendLine($"        setV2Props(v2Keys.{typeName}, props)");
            tsSb.AppendLine("        // Server-owned prefetch plan (opt-in, keeps TSX pure).");
            tsSb.AppendLine("        // Prefetch only Deferred/Lazy; Stream/SSE never auto-start.");
            foreach (var expr in CollectBehaviorExpressions(compilation, c))
            {
                tsSb.AppendLine($"        prefetchBehaviors({expr})");
            }
            tsSb.AppendLine("        return null");
            tsSb.AppendLine("    },");
            tsSb.AppendLine("    component: () => {");
            tsSb.AppendLine($"        useV2Props(v2Keys.{typeName})");
    ...
    private static IReadOnlyList<string> CollectBehaviorExpressions(Compilation compilation, ComponentInfo component)
    {
        // Walk the C# props model to find Component<T>.Behaviors occurrences and emit TS expressions like:
        //   props.agentPanel.behaviors
        //   props.foo.bar.behaviors
        var root = compilation.GetTypeByMetadataName($"{component.Namespace}.{component.TypeName}") as INamedTypeSymbol;
        if (root is null) return Array.Empty<string>();

        var componentDef = compilation.GetTypeByMetadataName("Shalimar.Component`1") as INamedTypeSymbol;
        if (componentDef is null) return Array.Empty<string>();

        var output = new List<string>();
        Collect(root, tsPrefix: "props", componentDef, output);
        ...
        static void Collect(INamedTypeSymbol propsType, string tsPrefix, INamedTypeSymbol componentDef, List<string> output)
        {
            foreach (var p in propsType.GetMembers().OfType<IPropertySymbol>())
            {
                ...
                {
                    var def = named.ConstructedFrom;
                    if (SymbolEqualityComparer.Default.Equals(def, componentDef))
                    {
                        // behaviors at this level
                        output.Add($"{tsPrefix}.{tsName}.behaviors");

                        // descend into child props via .props
                        if (named.TypeArguments.Length == 1 && named.TypeArguments[0] is INamedTypeSymbol childProps)
                            Collect(childProps, $"{tsPrefix}.{tsName}.props", componentDef, output);
                    }
                }
            }
        }
    ...
    private static void GenerateTypeScriptV2StoreEmbedded(
        SourceProductionContext context,
        Compilation compilation,
        List<ComponentInfo> components)
    {
        ...
        var tsSb = new StringBuilder();
        tsSb.AppendLine("// Generated by Shalimar - DO NOT EDIT");
        tsSb.AppendLine("import { Store } from '@tanstack/store'");
        tsSb.AppendLine("import { useStore } from '@tanstack/react-store'");
        tsSb.AppendLine("import type {");
        foreach (var c in v2)
            tsSb.AppendLine($"    {c.TypeName},");
        tsSb.AppendLine("} from './shalimar-types.g'");
        tsSb.AppendLine();
        tsSb.AppendLine("export type V2PropsByComponent = {");
        foreach (var c in v2)
            tsSb.AppendLine($"    {c.TypeName}: {c.TypeName}");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export const v2Keys = {");
        foreach (var c in v2)
            tsSb.AppendLine($"    {c.TypeName}: '{c.TypeName}',");
        tsSb.AppendLine("} as const");
        tsSb.AppendLine();
        tsSb.AppendLine("export type V2ComponentKey = keyof typeof v2Keys");
        tsSb.AppendLine();
        tsSb.AppendLine("type V2RouteState<TProps> = {");
        tsSb.AppendLine("    props: TProps | null");
        tsSb.AppendLine("    // UI state is explicitly separated so invalidations/props refresh never nukes it.");
        tsSb.AppendLine("    ui: Record<string, unknown>");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export const v2Stores: { [K in V2ComponentKey]: Store<V2RouteState<V2PropsByComponent[K]>> } = {");
        foreach (var c in v2)
            tsSb.AppendLine($"    {c.TypeName}: new Store<V2RouteState<V2PropsByComponent['{c.TypeName}']>>({{ props: null, ui: {{}} }}),");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function setV2Props<K extends V2ComponentKey>(key: K, props: V2PropsByComponent[K]) {");
        tsSb.AppendLine("    v2Stores[key].setState((s) => ({ ...s, props }))");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function useV2Props<K extends V2ComponentKey>(key: K): V2PropsByComponent[K] {");
        tsSb.AppendLine("    const state = useStore(v2Stores[key])");
        tsSb.AppendLine("    if (!state.props) throw new Error(`Missing v2 props for ${String(key)}. Did the route loader run?`)");
        tsSb.AppendLine("    return state.props as V2PropsByComponent[K]");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function getV2Props<K extends V2ComponentKey>(key: K): V2PropsByComponent[K] | null {");
        tsSb.AppendLine("    return (v2Stores[key].state.props as V2PropsByComponent[K] | null) ?? null");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function useV2Ui<K extends V2ComponentKey>(key: K): Record<string, unknown> {");
        tsSb.AppendLine("    return useStore(v2Stores[key]).ui");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        tsSb.AppendLine("export function mergeV2Ui<K extends V2ComponentKey>(key: K, patch: Record<string, unknown>) {");
        tsSb.AppendLine("    v2Stores[key].setState((s) => ({ ...s, ui: { ...s.ui, ...patch } }))");
        tsSb.AppendLine("}");
        tsSb.AppendLine();
        ...
        csSb.AppendLine("// <auto-generated/>");
        csSb.AppendLine("/*");
        csSb.AppendLine("SHALIMAR_TS: shalimar-v2-store.g.ts");
        csSb.Append(tsSb);
        csSb.AppendLine("END_SHALIMAR_TS");
        csSb.AppendLine("*/");
        ...
        List<ComponentInfo> components)
    {
        ...
        tsSb.AppendLine("    useDeferred,");
        tsSb.AppendLine("    useLazy,");
        tsSb.AppendLine("    useStream,");
        tsSb.AppendLine("    useSse,");
        tsSb.AppendLine("    invalidateDeferredByPrefix,");
        tsSb.AppendLine("    invalidateLazyByPrefix,");
        tsSb.AppendLine("    invalidateStreamedByPrefix,");
        tsSb.AppendLine("    invalidateSseByPrefix,");
        tsSb.AppendLine("} from '@shalimar/runtime'");
        tsSb.AppendLine("import { useV2Props, getV2Props, v2Keys } from './shalimar-v2-store.g'");
        tsSb.AppendLine();

        foreach (var c in v2)
        {
            tsSb.AppendLine($"export function use{c.TypeName}() {{");
            tsSb.AppendLine($"    return useV2Props(v2Keys.{c.TypeName})");
            tsSb.AppendLine("}");
            tsSb.AppendLine();
        }
        ...
        foreach (var leaf in leaves)
        {
            ...
            tsSb.AppendLine($"    return useV2Props(v2Keys.{leaf.RootTypeName}).{leaf.TsAccessPathToHandle}.href");
            tsSb.AppendLine("}");
            tsSb.AppendLine();
        }

        // Convenience data hooks for Deferred leaves (keeps TSX “pure”: no href wiring, no direct useDeferred).
        ...
        // Leaf invalidation helpers (by href prefix, includes query-string variants).
        foreach (var leaf in leaves)
        {
            var fn = $"invalidate{leaf.RootTypeName}{leaf.FunctionSuffixPascal}";
            tsSb.AppendLine($"export function {fn}() {{");
            tsSb.AppendLine($"    const props = getV2Props(v2Keys.{leaf.RootTypeName})");
    ...
    private static string MapV2RouteModuleFile(string normalizedPath)
    {
        ...
        if (normalizedPath == "/")
            return "V2Routes/Home/route.tsx";
        ...
        return $"V2Routes/{string.Join("/", folders)}/route.tsx";
    }

    private static string ToGeneratedPath(string embeddedUnderGenerated)
    {
        // `embeddedUnderGenerated` is relative to the Generated output directory (e.g. "V2Routes/V2/route.tsx").
        // Route config paths are project-root relative, so we prefix with "Generated/".
        return "Generated/" + embeddedUnderGenerated.TrimStart('/');
    }
